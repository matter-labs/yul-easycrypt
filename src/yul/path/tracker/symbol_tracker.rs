...
impl<T> Default for SymbolTracker<T>
where
    T: Clone + std::fmt::Debug + PartialEq + Eq,
{
    fn default() -> Self {
...
{
    ///
    /// Add a key-value pair to the tracker at the current position.
    ///
    pub fn add(&mut self, name: &String, value: &T) {
        let _ = self.symbols.add(name, value);
    }
...
{
    ///
    /// Retrieves a value by name. The lookup starts at the current scope, then
    /// proceeds to its parents until it reaches the root scope.
    ///
